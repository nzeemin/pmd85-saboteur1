...
LD210:	DEFB $04,$CE,$09,$63,$09,$63,$09,$63,$09,$63
	...
	ALIGN 256
; Mirror table - adopted for PMD85, mirrors 6 lower bits
MIRROR:
	DEFB $00,$20,$10,$30,$08,$28,$18,$38,$04,$24,$14,$34,$0C,$2C,$1C,$3C
	DEFB $02,$22,$12,$32,$0A,$2A,$1A,$3A,$06,$26,$16,$36,$0E,$2E,$1E,$3E
	DEFB $01,$21,$11,$31,$09,$29,$19,$39,$05,$25,$15,$35,$0D,$2D,$1D,$3D
	DEFB $03,$23,$13,$33,$0B,$2B,$1B,$3B,$07,$27,$17,$37,$0F,$2F,$1F,$3F
	DEFB $40,$60,$50,$70,$48,$68,$58,$78,$44,$64,$54,$74,$4C,$6C,$5C,$7C
	DEFB $42,$62,$52,$72,$4A,$6A,$5A,$7A,$46,$66,$56,$76,$4E,$6E,$5E,$7E
	DEFB $41,$61,$51,$71,$49,$69,$59,$79,$45,$65,$55,$75,$4D,$6D,$5D,$7D
	DEFB $43,$63,$53,$73,$4B,$6B,$5B,$7B,$47,$67,$57,$77,$4F,$6F,$5F,$7F
	DEFB $80,$A0,$90,$B0,$88,$A8,$98,$B8,$84,$A4,$94,$B4,$8C,$AC,$9C,$BC
	DEFB $82,$A2,$92,$B2,$8A,$AA,$9A,$BA,$86,$A6,$96,$B6,$8E,$AE,$9E,$BE
	DEFB $81,$A1,$91,$B1,$89,$A9,$99,$B9,$85,$A5,$95,$B5,$8D,$AD,$9D,$BD
	DEFB $83,$A3,$93,$B3,$8B,$AB,$9B,$BB,$87,$A7,$97,$B7,$8F,$AF,$9F,$BF
	DEFB $C0,$E0,$D0,$F0,$C8,$E8,$D8,$F8,$C4,$E4,$D4,$F4,$CC,$EC,$DC,$FC
	DEFB $C2,$E2,$D2,$F2,$CA,$EA,$DA,$FA,$C6,$E6,$D6,$F6,$CE,$EE,$DE,$FE
	DEFB $C1,$E1,$D1,$F1,$C9,$E9,$D9,$F9,$C5,$E5,$D5,$F5,$CD,$ED,$DD,$FD
	DEFB $C3,$E3,$D3,$F3,$CB,$EB,$DB,$FB,$C7,$E7,$D7,$F7,$CF,$EF,$DF,$FF
...
L6289:	LD DE,L62DB	; ZX0 encoded picture data address
...
L62A9:	LD HL,SCRMP	; Screen address
 	...
 	LD C,16		; Number of columns
L62B1: 	PUSH HL		; save screen address
 	PUSH BC		; save counter C
 	LD B,185	; Number of lines
...
.skipc:	ld L,A
	dec B
 	jp nz,L62B8	; loop by lines
 	POP BC		; restore counter C
...
; Initial Energy fill
L7472:	ld de,SCRIND+$0386 ; screen address
	ld b,15
L7481:	push de
	ld c,10
L7485:	ld a,$3F	; filler
...
PRSTR:	LD A,(HL)	; get symbol byte
	...
	add HL,HL	; * 4
	add HL,DE	; * 6
LAEDA:	ld de,FONT-$20*6 ; font address
	ADD HL,DE	; HL = symbol address
...
DRTILE_FF:
	...
	ld (hl),E	; inversed $DF
	add HL,DE	; next line
	ld (hl),$60	; inversed $FB
	add HL,DE	; next line
	ld (hl),E	; inversed $DF
	add HL,DE	; next line
	ld (hl),E	; inversed $FF
	add HL,DE	; next line
	ld (hl),$48	; inversed $F7
	add HL,DE	; next line
	ld (hl),E	; inversed $FF
	jp LB2A6
...
LB7A4:	LD HL,LAD57	; Indicator Time value address
	...
	DEFW SCRIND+$0197; screen address for timer value
	DEFB 2		; Two digits
; Check for BOMB
...
LB891:	LD HL,LB84F	; NEAR item address
	...
	JP Z,LB8B0
	LD (HL),A
	ld HL,SCRIND+$001B ; NEAR screen address
...
LB8B0:	LD HL,LB850	; HELD tile address
	...
	SUB $C6
	rra	; SRL A
LB8C2:	ld hl,SCRIND+$0000 ; HELD screen address
	CALL DRITEM	; Draw NEAR/HELD item
LB8C9:	XOR A
	LD (LA39E),A
...
ReadInput:
; Test keys: Space, Q, A
	xor A		; keyboard row 0
	out (SysPA),A
	in A,(SysPB)	; A bits: Space A Q 1 K0
	cpl
	ld C,A
	and $10		; FIRE bit for Space key
	ld B,A
	ld A,C
	rla
	and $08		; UP bit for Q key
	or B
	ld B,A
	ld A,C
	rra
	and $04		; DOWN bit for A key
	or B
	ld B,A
; Test keys: <, O, 9
	ld A,8		; keyboard row 8
	out (SysPA),A
	in A,(SysPB)	; A bits: < L O 9 K8
	cpl
	and $16		; keys: < O 9
	jp z,.l20
	ld A,$02	; LEFT bit
...
.l20:
; Test keys: >, P, 0
	ld A,9		; keyboard row 9
	out (SysPA),A
	in A,(SysPB)	; A bits: > ; P 0 K9
	cpl
	and $16		; keys: > P 0
	jp z,.l30
	ld A,$01	; RIGHT bit
...
LC2BB:	ld a,(INPUTB)
	...
	in A,(SysPC)
	or 4
LC2E2:	out (SysPC),A	; sound of steps
	xor 4
	dec b
	jp nz,LC2E2
LC2E8:	LD H,$00
...
LF9A1:	ld a,b
	...
	rla
	ld b,a
	LD HL,SABOT1CODE_START	; address where to get delay values
	in A,(SysPC)
LF9AC:	out (SysPC),A
	xor 4
	LD C,(HL)	; get delay
...
; Menu picture (two ninjas), ZX0 encoded, 503 bytes
L62DB:	INCBIN "sabot1mp.zx0"

; Front tiles, 124 tiles, 12 bytes each
	INCLUDE "sabot1t1.asm"
Sabot1Tiles1End:	; Gap of 118*12 bytes starts here
...
Sabot1Tiles2End:	; Gap of 30*8 bytes starts here
...
Sabot1Tiles2Gap EQU Sabot1Tiles2B - Sabot1Tiles2End
	...
	ASSERT Sabot1Tiles2Gap == 240  ; Make sure second part of tiles properly aligned
